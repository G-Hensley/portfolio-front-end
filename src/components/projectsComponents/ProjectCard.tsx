...
export default function ProjectCard( {title, image, link, skills, description }: { title: string, image: string, link: string, skills: React.ReactNode[], description: string } ) {
    return (
        <div className="project-card relative w-full sm:w-4/5 lg:w-[45%] 2xl:w-[31%] h-auto sm:py-0">
            ...
            <a href={link} target='_blank' className="project-info-container items-center justify-center flex-col gap-4 h-full w-full absolute left-0 top-0 p-4">
                <p className="project-description text-[1rem] sm:text-desktop-body font-body-ff text-center relative text-primary-cyan-900">
                    {description}
                </p>
                <p className="tools-used text-primary-magenta-700 text-[1rem] sm:text-desktop-body font-h2-ff text-center relative">Tools Used</p>
                ...
                <p className="click-to-view animate-bounce text-primary-cyan-900 text-[1rem] sm:text-desktop-body font-h2-ff text-center relative w-full">Click to View Project</p>
            </a>
        </div>
    );
}
